...
class R_Actor(nn.Module):
    ...
    def forward(self,
                ...
                active_masks=None,
                evaluate=False):
        """Compute actions, or evaluate given actions when ``evaluate=True``.

        DDP only intercepts ``forward``, so evaluation must go through this
        path rather than a separate ``evaluate_actions`` method.
        """
        ...
        if evaluate:
            if action is None:
                raise ValueError("evaluate=True requires an action tensor")

...
\begin{figure*}[t]
  ...
  \caption{Held-out results for the evaluated DDQN checkpoint. Left: goal
...
The available evaluation artifact does not contain per-episode agent counts
or occlusion-severity values, so collision-vs-density and collision-vs-occlusion
curves cannot be recovered faithfully. Figure~\ref{fig:curves} instead plots
the available 1{,}000-episode moving-average reward and exploration schedule
from the completed DDQN run. Goal and collision rates were not emitted by the
training logger and are reported only for deterministic evaluation rather
than reconstructed from reward.

...
The completed DDQN run used synchronous vectorization with four workers and
12 environments per worker (48 parallel environments), a training frequency
of 128, a replay buffer of $10^6$, batch size 256, and the hyperparameters of
Section~\ref{sec:agent}; the full configuration is in
Appendix~\ref{app:hyper}. The supplied log reaches episode 347{,}000,
11{,}757{,}888 environment steps, and 22{,}955 learner updates at approximately
283 environment frames per second. The EC2 instance type, wall-clock duration,
and hardware inventory were not included in the supplied log.
...
The console trace records a clear improvement in the exploration-policy
return over the completed run. During episodes 1{,}000--10{,}000, the ten
...
The run also demonstrates stable collection throughput: frames per second
...
are therefore not substitutes for deterministic held-out evaluation. The
separate 1{,}500-episode evaluation below provides that test.
...
complements the held-out analysis with the available 1{,}000-episode moving
average reward and exploration schedule from the completed DDQN run. Goal
and collision rates were not emitted by the training logger and are therefore
reported only for deterministic evaluation rather than reconstructed from
reward.
...
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.9\linewidth}{\centering\vspace{2.8cm}
\todofig{Plot the supplied DDQN training log: 1{,}000-episode average reward
vs.\ environment steps on the left axis and epsilon vs.\ environment steps
on the right axis. Mark replay-buffer saturation at episode 36{,}000 and the
late-training interval (episodes 331{,}000--347{,}000). Do not plot training
goal/collision curves because those fields were not logged.}
  \vspace{2.8cm}}}

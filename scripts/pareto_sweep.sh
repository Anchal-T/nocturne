...
for penalty in "${PENALTIES[@]}"; do
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [DRY RUN] python -m examples.drl_collision_avoidance.train seed=${SEED} reward.collision_penalty=${penalty} drl.num_episodes=${NUM_EPISODES} drl.checkpoint_dir=${CKPT_DIR}"
    else
